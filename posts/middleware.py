...
class PostsMiddleware(MiddlewareMixin):
    def process_request(self, request):
        ...
        posts_games = Post.objects.filter(type='games', active=True)

        # On récupère tous les posts de type 'movies'
        posts_movies = Post.objects.filter(type='movies', active=True)

        # On récupère tous les posts de type 'music'
        posts_music = Post.objects.filter(type='music', active=True)

        # On récupère tous les posts de type 'tech'
        posts_tech = Post.objects.filter(type='tech', active=True)
